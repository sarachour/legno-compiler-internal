...
void setup() {
  fabric = new Fabric();
  fabric->calibrate();
  Serial.println("dyn_range\txinp_real\txinp_imag\tyinp_real\tyinp_imag\tctrl_real\tctrl_imag\texpr_real\texpr_imag");
}
...
void loop () {
  for (unsigned char chipIndx = 0; chipIndx < 2; chipIndx++) {
    for (unsigned char tileIndx = 0; tileIndx < 4; tileIndx++) {
      ...
      Serial.print( dynR, 6 ); Serial.print('\t');
      ...
      testTile.setDynRangeXY ( dynR, xr, xi, yr, yi );

      fabric->cfgCommit();
      float mr = fabric->chips[chipIndx].tiles[3].slices[2].chipOutput->analogAvg(CAL_REPS) * dynR / FULL_SCALE;
      float mi = fabric->chips[chipIndx].tiles[3].slices[3].chipOutput->analogAvg(CAL_REPS) * dynR / FULL_SCALE;
