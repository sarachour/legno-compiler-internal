...
\item\tx{enable}: Determines whether the block is enabled (true) or disabled
  (false).
\item\tx{range}: Configures the current limit of the analog input. If the code
  is set to \tx{RANGE_MED}, the input must fall within $[-2 \mu A, 2 \mu A]$. If
  the code is set to \tx{RANGE_HIGH}, the input must fall within $-20 \mu A, 20
  \mu A$. This code cannot be set to \tx{RANGE_LOW}
