...
#DATAGEN = anomaly.EarthquakeRNNAnomaly(8)
DATAGEN = anomaly.HeartRNNAnomaly(8)
def test_network(winner_net):
    datagen = DATAGEN
    num_correct = 0
    num_total = 0
    for test_no,(seq,clazz) in enumerate(datagen \
...
def eval_genome(genome,config):
    ...
    datagen = DATAGEN
    errors = {0:0,1:0}
    for seq,clazz in datagen.generate_train_dataset():
        net.reset()
        for s in seq:
            output = net.activate([s])
            errors[clazz] += datagen.error(clazz,output[0]) ** 2

    error = errors[0] + errors[1]
    return 4.0 - 4.0 * (error / datagen.size)
